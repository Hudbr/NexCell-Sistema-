...
function mount(){
  ...
  if(!nav.querySelector('[data-view="cash"]')){
    nav.insertAdjacentHTML('beforeend','<button class="mobile-link" type="button" data-view="cash"><span>💵</span><span>Caixa</span></button>');
  }
...
function syncActive(view){
  ...
  const more=$('mobileMoreButton');if(more)more.classList.toggle('active',MORE.some(([name])=>name===view));
}
function wire(){
